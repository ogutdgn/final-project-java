        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(CARS_FILE))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length >= 5) {
                    ...
                    carData.makeModel = parts[0];
                    carData.plateNumber = parts[1];
                    carData.year = Integer.parseInt(parts[2]);
                    carData.price = Integer.parseInt(parts[3]);
                    carData.available = Boolean.parseBoolean(parts[4]);
                    carData.ownerUsername = parts.length > 5 && !parts[5].equals("NONE") ? parts[5] : null;
                    carData.renterUsername = parts.length > 6 && !parts[6].equals("NONE") ? parts[6] : null;
